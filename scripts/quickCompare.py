...
from ROOT import gROOT, gPad, gDirectory, TFile, TH1D, TCanvas, kBlue, kRed
...
def compareFiles(filename1,filename2,variables,treename='Events',outdir=".",tag=""):
  """Compare distributions in two given files."""
  print ">>> compareFiles"
  ...
  print ">>> %10s, %s"%(tree1.GetEntries(),filename1)
  print ">>> %10s, %s"%(tree2.GetEntries(),filename2)
  ...
  for var, nbins, xmin, xmax in variables:
    ...
    name1 = var+'_1'
    name2 = var+'_2'
    hist1 = TH1D(name1,var,nbins,xmin,xmax)
    hist2 = TH1D(name2,var,nbins,xmin,xmax)
    out1  = tree1.Draw("%s >> %s"%(var,name1))
    out2  = tree1.Draw("%s >> %s"%(var,name2))
    print ">>>   %s: %s vs. %s entries"%(var,out1,out2)
...
#      "/pnfs/psi.ch/cms/trivcat/store/user/ineuteli/samples/NANOAOD_2018/DYJetsToLL_M-10to50_TuneCP5_13TeV-madgraphMLM-pythia8/RunIIAutumn18NanoAODv4-Nano14Dec2018_102X_upgrade2018_realistic_v16-v1/NANOAODSIM/BBDE47BF-CEA4-154A-B713-56480CA9C97F_skimmed.root"),
  ...
  for sample, file1, file2 in files:
      ...
      compareFiles(file1,file2,variables,tag=tag,outdir=outdir)  
